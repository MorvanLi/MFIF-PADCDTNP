...
class Evaluator():
    ...
    def MSE(cls, image_F, image_A, image_B):  # MSE
        ...
        m, n = image_F.shape
        MSE_AF = np.sum(np.sum((image_F - image_A) ** 2)) / (m * n)
        MSE_BF = np.sum(np.sum((image_F - image_B) ** 2)) / (m * n)
        return 0.5 * MSE_AF + 0.5 * MSE_BF
    ...
    def SCD(cls, image_F, image_A, image_B): # The sum of the correlations of differences
        ...
        # return corr1 + corr2
        a = corr2(image_F - image_B, image_A)
        b = corr2(image_F - image_A, image_B)
